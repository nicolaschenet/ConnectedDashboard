...
var settings = require('./instagram_settings');
...
function processGeography(geoName, update){
    ...
    getMinID(geoName, function(error, minID){
        ...
        var options = {
            host: settings.IG_API_HOST,
            // Note that in all implementations, basePath will be ''. Here at
            // instagram, this aint true ;)
            path: settings.IG_BASE_PATH + path + queryString
        };
        if(settings.IG_API_PORT){
            options['port'] = settings.IG_API_PORT;
        }

        // Asynchronously ask the Instagram API for new media for a given
        // geography.
        settings.IG_HTTP_CLIENT.get(options, function(response){
            var data = '';
...
function getMedia(callback){
    ...
    r.lrange('media:objects', 0, 8, function(error, media){
